// Worker Cloudflare — arome-proxy v4
// L'API MF utilise le type de requête dans le PATH, pas en paramètre request=
// GetCapabilities → /GetCapabilities?apikey=...
// GetMap          → /GetMap?layers=...&apikey=...
...
export default {
  async fetch(request) {
    ...
    // Extraire le type de requête WMS (GetCapabilities, GetMap...)
    const requestType = params.get('request') || 'GetCapabilities'
    params.delete('request')
    params.delete('apikey')
    params.set('apikey', MF_KEY)

    // L'API MF attend le type en PATH, pas en paramètre
    const mfUrl = MF_BASE + '/' + requestType + '?' + params.toString()
